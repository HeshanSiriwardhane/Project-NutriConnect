...
//    $order_query = mysqli_query($conn, "SELECT * FROM `donation_orders` WHERE name = '$name'  AND email = '$email' AND phone = '$phone' AND address = '$address' AND cname = '$cname' AND cnum = '$cnum' AND month = '$month' AND year = '$year' AND cvv = '$cvv' AND total_products = '$total_products' AND total_price = '$cart_total'") or die('query failed');
...
//          $message[] = 'Order Already Placed!'; 
//       }else{
//          mysqli_query($conn, "INSERT INTO `donate_orders`(user_id, name, email, phone, address, cname, cnum, month, year, cvv, total_products, total_price, placed_on) VALUES('$user_id', '$name', '$phone', '$email', '$address', '$cname', '$cnum', '$month', '$year', '$cvv','$total_products', '$cart_total', '$placed_on')") or die('query failed');
//          $message[] = 'Order Placed Successfully!';
//          mysqli_query($conn, "DELETE FROM `d_cart` WHERE user_id = '$user_id'") or die('query failed');
//       }
//    }
...
if (isset($_POST['order_btn'])) {
   echo "<script>alert('Successfully Donated !! Thank you for join with our Donation Fund!!')</script>";
}
...
<div class="headers">
        ...
        <a class="active" href="home.php">Home</a>
            ...
            <a href="donate_orders.php">Donate Orders</a>
        </nav>
...
<div class="container">
    ...
    <form action="" method="post">
        ...
        <input type="submit" value="Donate Now" class="submit-btn" name="order_btn" onclick="return confirm('Do you need to confirm Donation?');">
